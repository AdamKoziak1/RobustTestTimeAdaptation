...
class PolicyPredictor(nn.Module):
    def __init__(self, n_sub_policy, sub_policy_dim, name="default") -> None:
        ...
        self.register_parameter(
            "policy_selection_weights",
            torch.nn.parameter.Parameter(torch.zeros(n_sub_policy)),
        )
        self.register_parameter(
            "policy_mag_weights",
            torch.nn.parameter.Parameter(torch.zeros(n_sub_policy, sub_policy_dim)),
        )

    def reset_weights(self):
        self.policy_selection_weights.data.zero_()
        self.policy_mag_weights.data = torch.zeros(
            (self.n_sub_policy, self.sub_policy_dim),
            device=self.policy_mag_weights.data.device,
        )
