...
class TTA3(nn.Module):
    ...
    def __init__(self, model, optimizer, steps=1, episodic=False, 
                 ...
                 r=4, 
                 l_adv_iter=1,  cr_type='cosine', cr_start=0, use_mi=False, lam_em=0.0):
        super().__init__()
        self.model = model
        self.optimizer = optimizer
        self.steps = steps
...
# --- Consistency Regularization Loss (L_CR) ---    
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        if self.lambda_nuc > 0.0 and hasattr(model.featurizer, "pop_nuc_penalty"):
            L_NUC = model.featurizer.pop_nuc_penalty()

        # --- Overall Loss ---
        loss = (self.lam_em * base_loss) + (self.lambda1 * L_Flat) + (self.lambda2 * L_Adv) + (self.lambda3 * L_CR) + (self.lambda4 * L_PL) + (self.lambda_nuc * L_NUC)
